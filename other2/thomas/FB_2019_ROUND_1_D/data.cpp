...
int main()
{
    ...
    for (int fileId = 1; fileId <= FileCount; ++fileId)
    {
        ...
        while (T--)
        {
            int N = rdNum.GetRand(2, 1e1);
            int H = rdNum.GetRand(0, N);
            int V = rdNum.GetRand(0, N);

            fout << N << " " << H << " " << V << "\n";

            fout << rdNum.GetRand(0, 1e1) << " " << rdNum.GetRand(0, 1e1) << " " 
                 << rdNum.GetRand(0, 1e1) << " " << rdNum.GetRand(0, 1e1) << " " << rdNum.GetRand(0, 1e1) << " "
                 << rdNum.GetRand(1, 1e1) << "\n";
