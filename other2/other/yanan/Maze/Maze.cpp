...
int main()
{
    ...
    for (long long i = 0;i<n;++i)
    {
        ...
        while(true)
        {
            ...
            if (out(TQ.X - 1, TQ.Y, x, y) == false && value[TQ.X - 1][TQ.Y] > maze[TQ.X - 1][TQ.Y] + value[TQ.X][TQ.Y])
            {
                point temp;
                temp.X = TQ.X - 1;
                temp.Y = TQ.Y;
                temp.Value = maze[temp.X][temp.Y];
                value[temp.X][temp.Y] = temp.Value + value[TQ.X][TQ.Y];
                temp.Value = value[temp.X][temp.Y];
                pq.push(temp);
            }
            if (out(TQ.X + 1, TQ.Y, x, y) == false && value[TQ.X + 1][TQ.Y] > maze[TQ.X + 1][TQ.Y] + value[TQ.X][TQ.Y])
            {
            ...
            if (out(TQ.X, TQ.Y - 1, x, y) == false && value[TQ.X][TQ.Y - 1] > maze[TQ.X][TQ.Y - 1] + value[TQ.X][TQ.Y])
            {
                point temp;
                temp.X = TQ.X;
                temp.Y = TQ.Y-1;
