...
void dfs(int u, int &cnt, int &cntScc, vector<int> &topLog, vector<int> &ids, vector<int> &low, vector<int> &inStack, stack<int> &stk, vector<unordered_set<int>> &g)
{
...
void solve()
{
    ...
    for (int c = 1; c <= totC; ++c)
    {
        ...
        g[get0(sta1)].insert(get1(sta2));
        g[get0(sta2)].insert(get1(sta1));
    }
    ...
    for (int c = 1; c <= totI; ++c)
    {
        ...
        g[get1(sta1)].insert(get0(sta2));
        g[get1(sta2)].insert(get0(sta1));
    }
    ...
    for (int i = 0; i <= n - 1; ++i)
    {
        ...
        while (ite != fre.end())
        {
            ...
            g[get1(now.num)].insert(get0(oth.num));
            g[get1(oth.num)].insert(get0(now.num));

            ++ite;
        }
    }
    cerr << "input:" << t.elapsed() << endl; //打印秒
    t.reset();
    int cnt = 0, cntScc = 0;
