...
ifstream fin("towerOfCubes_UVA10051.in");
ofstream fout("towerOfCubes_UVA10051.out");
...
int main()
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i <= n - 1; ++i)
        {
            fin >> a[i].front >> a[i].back >> a[i].left >> a[i].right >> a[i].top >> a[i].bottom;
        }
