...
ifstream fin("theSameGame_UVA758.in");
ofstream fout("theSameGame_UVA758.out");
...
int main()
{
    ...
    for (int c = 1; c <= testCase; ++c)
    {
        ...
        while (true)
        {
            ...
            fout << "Move " << move << " at (" << 10 - posY << ',' << posX + 1 << "): removed " << maxCount << " balls of color " << color << ", got " << (maxCount - 2) * (maxCount - 2) << " points.\n";
            finalScore += (maxCount - 2) * (maxCount - 2);

            // fout << posY << ' ' << posX << ' ' << maxCount << '\n';
            // for (int y = 0; y <= 9; ++y)
            // {
            //     for (int x = 0; x <= 14; ++x)
            //     {
        ...
        fout << "Final score: " << finalScore << ", with " << remaining << " balls remaining.\n";
    }
