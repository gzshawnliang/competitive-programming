    ...
    void solve()
    {
        ...
        for (ill tcc = 1; tcc <= tct; ++tcc)
        {
            ...
            for (ill i = 0; i <= n - 1; ++i)
            {
                ...
                if (i == 0)
                {
                    maxValues.push_back({preSum[i], i});
                    nowMaxV = maxValues.back().first;
                }
                else if (preSum[i] > nowMaxV)
                {
                    maxValues.push_back({preSum[i], i});
                    nowMaxV = maxValues.back().first;
                }
            ...
            for (ill q = 1; q <= tolQ; ++q)
            {
                if (q > 1) cout << ' ';
                ill input; cin >> input;

                auto tmpPos = lower_bound(maxValues.begin(), maxValues.end(), make_pair(input, -1LL)) - maxValues.begin();
                if (tmpPos == sizMV)
                {
                    if (preSum.back() <= 0)
                    {
                    ...
                    else
                    {
                        ...
                        ill left = input - cntLoops * preSum.back();
                        auto tmpPos2 = lower_bound(maxValues.begin(), maxValues.end(), make_pair(left, -1LL)) - maxValues.begin();
